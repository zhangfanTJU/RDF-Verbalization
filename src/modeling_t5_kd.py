...
from modeling_t5_kg import KgT5Block
from torch.nn.parameter import Parameter


class KdT5Stack(T5PreTrainedModel):
    def __init__(self, config, embed_tokens, embed_tokens_k):
        super().__init__(config)
        config_ = copy.deepcopy(config)
        config_.num_layers = 4 if config.num_layers == 12 else 6
        self.encoder_d = T5Stack(config_, embed_tokens_k)
        ...
        if self.use_unk:
            self.unk_emb = Parameter(torch.Tensor(1, config.d_model))
            factor = self.config.initializer_factor
            self.unk_emb.data.fill_(factor * 1.0)
        ...
        self.kg_embeddings = nn.Embedding(ent_tot + rel_tot + 1, d_kg)
    ...
    def know_avg_pooling(self, hidden_states_k, attention_mask_k, index):
        ...
        if self.use_unk:
            ...
            hidden_size = hidden_states_k.shape[-1]
            z = torch.zeros((1, hidden_size)).to(hidden_states_avg.device)
    ...
    def forward(
        ...
        # hidden_states = hidden_states + hidden_states_d  # for one time
        ...
        for i, layer_module in enumerate(self.block):
            ...
            hidden_states = hidden_states + hidden_states_d
            hidden_states = (hidden_states, hidden_states_k)
...
class KdT5ForConditionalGeneration(T5PreTrainedModel):
    ...
    def __init__(self, config):
        ...
        shared_k = nn.Embedding(config.vocab_size, config.d_model)

        encoder_config = copy.deepcopy(config)
        encoder_config.use_cache = False
        encoder_config.is_encoder_decoder = False
        self.encoder = KdT5Stack(encoder_config, self.shared, shared_k)
