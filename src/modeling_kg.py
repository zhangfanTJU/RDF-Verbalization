...
class KgEncoderLayer(nn.Module):
    def __init__(self, config: BartConfig):
        ...
        self.kg_dim = config.d_kg
        self.kg_num_heads = config.kg_num_heads

        self.concat = config.concat
        if self.concat:
            self.self_attn_k = Attention(self.kg_dim, self.kg_num_heads, dropout=config.attention_dropout)
            self.self_attn_layer_norm_k = LayerNorm(self.kg_dim)

        self.fc1_k = nn.Linear(self.kg_dim, config.encoder_ffn_dim)
        self.fc2_k = nn.Linear(config.encoder_ffn_dim, self.kg_dim)
        self.final_layer_norm_k = LayerNorm(self.kg_dim)
    ...
    def forward(self, x, encoder_padding_mask, output_attentions=False):
        ...
        """
        x, x_k = x
        encoder_padding_mask, encoder_padding_mask_k = encoder_padding_mask
        ...
        if self.concat:
            # for x_k
            residual_k = x_k
            x_k, attn_weights_k = self.self_attn_k(
                query=x_k, key=x_k, key_padding_mask=encoder_padding_mask_k, output_attentions=output_attentions
            )
            x_k = F.dropout(x_k, p=self.dropout, training=self.training)
            x_k = residual_k + x_k
            x_k = self.self_attn_layer_norm_k(x_k)

        residual_k = x_k
        x_k = self.fc1_k(x_k)

        x_concat = self.activation_fn(x + x_k)
        x = x_concat
        x_k = x_concat
        ...
        # for x_k
        x_k = F.dropout(x_k, p=self.activation_dropout, training=self.training)
        x_k = self.fc2_k(x_k)
        x_k = F.dropout(x_k, p=self.dropout, training=self.training)
        x_k = residual_k + x_k
        x_k = self.final_layer_norm_k(x_k)

        x = (x, x_k)
        attn_weights = (attn_weights, attn_weights_k)

        return x, attn_weights


class KgBartEncoder(nn.Module):
    """
    ...
    def __init__(self, config: BartConfig, embed_tokens):
        ...
        if not hasattr(config, 'd_kg'):
            config.d_kg = 128
            config.kg_num_heads = 8

        d_kg = config.d_kg
        ent_tot, rel_tot = 2730, 354
        self.kg_embeddings = nn.Embedding(ent_tot + rel_tot + 2, d_kg, padding_idx=0)
    ...
    def forward(
        ...
        attention_mask = (attention_mask, attention_mask_k)
...
class KgBartModel(PretrainedBartModel):
    def __init__(self, config: BartConfig):
        ...
        self.encoder = KgBartEncoder(config, self.shared)
