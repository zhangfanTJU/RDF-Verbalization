...
class DefBartEncoder(nn.Module):
    """
    ...
    def __init__(self, config: BartConfig, embed_tokens, embed_tokens_k):
        super().__init__()
        self.encoder_k = BartEncoder(config, embed_tokens_k)
        ...
        if not hasattr(config, 'pooling_type'):
            config.pooling_type = 'cls'
        self.pooling_type = config.pooling_type

        if not hasattr(config, 'use_unk'):
            config.use_unk = False
    ...
    def know_cls_pooling(self, hidden_states_k, attention_mask_k, index):
        """
        Pooling knowledge represention by cls.
        """
        eos_idx = (~attention_mask_k).sum(-1, keepdim=True) - 1
        eos_mask = torch.zeros_like(attention_mask_k).scatter_(1, eos_idx, 1)

        hidden_states_cls = hidden_states_k[eos_mask, :]  # total_num, hidden_size

        if self.use_unk:
            hidden_states_cat = torch.cat((self.unk_emb, hidden_states_cls), 0)  # total_num + 1, hidden_size
        else:
            z = torch.zeros((1, 768)).to(hidden_states_cls.device)
            hidden_states_cat = torch.cat((z, hidden_states_cls), 0)
    ...
    def forward(
        ...
        # definition encode
        encoder_k_outputs = self.encoder_k(input_ids=input_ids_k, attention_mask=attention_mask_k, return_dict=return_dict)
        hidden_states_k = encoder_k_outputs.last_hidden_state

        # pooling
        attention_mask_k = invert_mask(attention_mask_k)

        if self.pooling_type == 'avg':
            hidden_states_k = self.know_avg_pooling(hidden_states_k, attention_mask_k, index)  # b x l x h
        elif self.pooling_type == 'cls':
            hidden_states_k = self.know_cls_pooling(hidden_states_k, attention_mask_k, index)
        ...
        # B x T x C -> T x B x C
        x = x.transpose(0, 1)
        # x = x + hidden_states_k  # for one time
...
class DefBartModel(PretrainedBartModel):
    def __init__(self, config: BartConfig):
        ...
        self.encoder = DefBartEncoder(config, self.shared, shared_k)
...
class DefBartForConditionalGeneration(PretrainedBartModel):
    base_model_prefix = "model"
    authorized_missing_keys = [r"final_logits_bias", r"encoder\.version", r"decoder\.version"]

    def __init__(self, config: BartConfig):
        super().__init__(config)
        base_model = DefBartModel(config)
